...
namespace seneca
{
    ...
    ostream& Publication::write(ostream& os) const
    {
        ...
        if (conIO(os))
        {
            os << "| "
               ...
               << setw(SENEC_SHELF_ID_LEN) << left << setfill(' ') << m_shelfId << " | ";

            if (strlen(m_title) > SENEC_TITLE_WIDTH)  // Print the box that contains only the max Title width
            {
                for (int i = 0; i < SENEC_TITLE_WIDTH; ++i)
                {
                    os << m_title[i];
                }
            }
            ...
            {
                os.fill('.');
                os << setw(SENEC_TITLE_WIDTH) << left << m_title;
            }
            os << " | ";
            os.fill(oldfill);
    ...
    istream& Publication::read(istream& is)
    {
        ...
        else
        {
            ...
            is.ignore();
            is.getline(id, SENEC_SHELF_ID_LEN + 1, '\t');  // Read shelfId from input, using tab
